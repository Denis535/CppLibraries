...
namespace std::extensions::tree_machine::pro {
    ...
    TEST(tests_02, test_00) { // NOLINT
        ...
        root->on_before_activate_callback().add([root, a, b]([[maybe_unused]] const any arg) {
            root->add_children(vector<node *>{a, b}, nullptr);
        });
        root->on_after_deactivate_callback().add([root]([[maybe_unused]] const any arg) {
    ...
    TEST(tests_02, test_01) { // NOLINT
        ...
        root->on_after_activate_callback().add([root, a, b]([[maybe_unused]] const any arg) {
            root->add_children(vector<node *>{a, b}, nullptr);
        });
        root->on_before_deactivate_callback().add([root]([[maybe_unused]] const any arg) {
            root->remove_children(nullptr, [](const auto *const child, [[maybe_unused]] const auto arg) { delete child; });
        });
